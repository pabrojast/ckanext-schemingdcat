  ...
  return {
    ...
    _updatePageNavigationState: function($button) {
       ...
       if (this._isCreatingNewDataset()) {
         ...
         if ($nameField.length && $titleField.length) {
           ...
           var currentTitle = $titleField.val();
             ...
             // simplemente generar un nombre único preventivamente
             if (currentTitle && (!currentName || this._seemsAutoGenerated(currentName, currentTitle))) {
               ...
               // Solo generar nombre único para evitar conflictos
               var timestamp = new Date().getTime();
               var uniqueName = potentialName + '-' + timestamp;
               $nameField.val(uniqueName);
               
               // Mostrar mensaje informativo simple
               this._showSimpleMessage('Se ha generado un nombre único para evitar conflictos: ' + uniqueName);
             }
         }
       }
     },
     
     // Funciones comentadas temporalmente para evitar errores 400
     // TODO: Re-implementar de forma más segura
     /*
     ...
     _handleExistingDraft: function(packageData) {
       // Función deshabilitada temporalmente
       console.log('Draft handling disabled temporarily');
     }
     */
   };
}); 