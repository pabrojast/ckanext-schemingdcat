...
from ckanext.schemingdcat.config import OGC2CKAN_HARVESTER_MD_CONFIG, OGC2CKAN_MD_FORMATS, DATE_FIELDS, DATASET_DEFAULT_FIELDS, RESOURCE_DEFAULT_FIELDS, CUSTOM_FORMAT_RULES
...
class SchemingDCATHarvester(HarvesterBase):
    ...
    def get_harvester_basic_info(self, config):
        ...
        if not config:
            ...
            raise ValueError(f'Configuration must be a JSON. Check README: {readme_doc}')
    ...
    def _get_local_required_lang(self):
        ...
        if self._local_required_lang is None:
            self._local_required_lang = self._local_schema.get('required_language', None)
    ...
    def _generate_identifier(self, dataset_dict):
        ...
        elif dataset_dict.get('inspire_id') and isinstance(dataset_dict['inspire_id'], str):
        ...
        if identifier_source:
            ...
            raise ValueError('Dataset identifier could not be generated. Need at least: inspire_id or title')
    ...
    def _validate_remote_schema(self, remote_ckan_base_url=None, remote_dataset_field_names=None, remote_resource_field_names=None, remote_dataset_field_mapping=None, remote_resource_field_mapping=None):
        """
        ...
        try:
            ...
            local_datasets_colnames = set(field['field_name'] for field in self._local_schema['dataset_fields'])
            local_distributions_colnames = set(field['field_name'] for field in self._local_schema['resource_fields'])
            ...
            if remote_ckan_base_url is not None:
                ...
                remote_datasets_colnames = set(field['field_name'] for field in self._remote_schema['dataset_fields'])
                remote_distributions_colnames = set(field['field_name'] for field in self._remote_schema['resource_fields'])       

            elif remote_dataset_field_names is not None:
                log.debug('Validating remote schema using field names from package dict')
            ...
            distributions_diff = local_distributions_colnames - remote_distributions_colnames

            def get_mapped_fields(fields, field_mapping):
                return [
                    {
                        'local_field_name': field['field_name'],
                        'remote_field_name': field_mapping.get(field['field_name'], field['field_name']) if field_mapping else field['field_name'],
                        'modified': field['field_name'] != (field_mapping.get(field['field_name'], field['field_name']) if field_mapping else field['field_name']),
                        **({'form_languages': field['form_languages']} if field.get('form_languages') else {}),
                        **({'required_language': field['required_language']} if field.get('required_language') else {})
                    } for field in fields
                ]

            self._mapped_schema = {
                'dataset_fields': get_mapped_fields(self._local_schema.get('dataset_fields', []), remote_dataset_field_mapping),
                'resource_fields': get_mapped_fields(self._local_schema.get('resource_fields', []), remote_resource_field_mapping)
            }

            log.info('Local required language: %s', self._local_required_lang)
            log.info('Field names differences in dataset: %s and resource: %s', datasets_diff, distributions_diff)
    ...
    def _find_existing_package_by_field_name(self, package_dict, field_name, return_fields=None):
        """
        ...
        if return_fields and isinstance(return_fields, list):
            data_dict['fl'] = ','.join(field if isinstance(field, str) and field == field.strip() else str(field).strip() for field in return_fields)

        package_search_context = {'model': model, 'session': Session, 'ignore_auth': True}
    ...
    def _check_existing_package_by_ids(self, package_dict):
        ...
        """
        basic_id_fields = ['name', 'id', 'identifier', 'alternate_identifier', 'inspire_id']
        for field_name in basic_id_fields:
            if package_dict.get(field_name):
                package = self._find_existing_package_by_field_name(package_dict, field_name)
    ...
    def _set_translated_fields(self, package_dict):
        ...
        """
        basic_fields = ['id', 'name', 'title', 'title_translated', 'notes_translated', 'provenance', 'notes', 'provenance', 'private', 'groups', 'tags', 'tag_string', 'owner_org']
        if not hasattr(self, '_mapped_schema') or 'dataset_fields' not in self._mapped_schema or 'resource_fields' not in self._mapped_schema:
            return package_dict
        try:
            translated_fields = {
                'dataset_fields': [],
            ...
            for field in self._mapped_schema['dataset_fields']:
                if field.get('modified', True):
                    ...
                    if isinstance(remote_field_name, dict):
                        package_dict[local_field_name] = {lang: package_dict.get(name, None) for lang, name in remote_field_name.items()}
                    else:
                ...
                for resource in package_dict['resources']:
                    for field in self._mapped_schema['resource_fields']:
                        if field.get('modified', True):
                            ...
                            if isinstance(remote_field_name, dict):
                                package_dict[local_field_name] = {lang: resource.get(name, None) for lang, name in remote_field_name.items()}
                            else:
    ...
    def _get_allowed_values(self, field_name, field_type='dataset_fields'):
        ...
        allowed_values = [choice['value'] for field in self._local_schema[field_type] 
                          if field['field_name'] == field_name for choice in field.get('choices', [])]
    ...
    def _create_or_update_package(self, package_dict, harvest_object,
        ...
        try:
            ...
            if existing_package_dict is not None:                                                   
                ...
                log.debug('existing_package_dict title: %s and ID: %s', existing_package_dict['title'], existing_package_dict['id'])
                ...
                package_dict['metadata_modified'] > existing_package_dict.get('metadata_modified'):
                    log.info('Package ID: %s with GUID: %s exists and needs to be updated', package_dict['id'], harvest_object.guid)
                    # Update package
                    context.update({'id': package_dict['id']})
                    
                    # Avoid adding resources if a resource with the same url already exists
                    existing_resource_ids = {resource['url'] for resource in existing_package_dict.get('resources', [])}
                    package_dict['resources'] = [resource for resource in package_dict.get('resources', []) if resource['url'] not in existing_resource_ids]
                ...
                log.info('Created new package ID: %s with GUID: %s', package_dict['id'], harvest_object.guid)
